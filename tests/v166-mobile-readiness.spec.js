...
test.describe("Mobile and cross-device readiness v1.6.6", () => {
  test.beforeEach(async ({ page }) => {
    await page.setViewportSize({ width: 390, height: 844 });
    await page.goto("/meeting.html");
    await page.evaluate(() => localStorage.clear());
    await page.reload();
    await expect(page.locator("#deviceReadinessV166")).toBeVisible();
  });
  ...
  test("phone viewport has no page-level horizontal overflow and exposes touch actions", async ({ page }) => {
    ...
    const dock = page.locator("#mobileActionDockV166");
  ...
  test("readiness report contains capability metadata but excludes meeting content", async ({ page }) => {
    const sensitiveMarker = "PRIVATE-MEETING-CONTENT-V166";
    await page.fill("#meetingTitle", sensitiveMarker);
    await page.fill("#notes", `${sensitiveMarker}-NOTES`);

    const report = await page.evaluate(() => window.collectDeviceReadinessV166());
    const serialized = JSON.stringify(report);

    expect(report.type).toBe("methodz-device-readiness-report");
    expect(report.appShellVersion).toBe("1.6.6");
    ...
    expect(report.boundaries.containsPrivateKeyMaterial).toBe(false);
  ...
  test("readiness actions remain explicit and mobile navigation reaches records", async ({ page }) => {
    await page.locator("#mobileActionDockV166 button[data-action='records']").click();
    await expect(page.locator("#savedRecordsSectionV166")).toBeInViewport();

    await page.locator("#mobileActionDockV166 button[data-action='readiness']").click();
    await expect(page.locator("#deviceReadinessV166")).toBeInViewport();

    await page.getByRole("button", { name: "Refresh Check" }).click();
    await expect(page.locator("#deviceReadinessMessageV166")).not.toHaveText("Checking this device...");
    await expect(page.locator("#deviceReadinessGridV166 .device-check-v166")).toHaveCount(7);
  });
});
