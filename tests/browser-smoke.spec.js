...
test("creates and saves a schema 1.5 meeting record", async ({ page }) => {
  ...
  expect(records[0].schemaVersion).toBe("1.5.0");
  ...
  expect(records[0].externalRecipientControl).toBeTruthy();
});
...
test("imports and migrates the original legacy storage key before workspace initialization", async ({ page }) => {
  ...
  expect(result.state.currentVersion).toBe("1.5.0");
...
test("publishes a valid app manifest and service worker entry point", async ({ request }) => {
  ...
  expect(await workerResponse.text()).toContain("methodz-meeting-manager-v1.5.0");
});
